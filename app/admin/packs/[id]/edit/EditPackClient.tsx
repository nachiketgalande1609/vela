...
export function EditPackClient({ pack, wallpapers }: { pack: PackData; wallpapers: Wallpaper[] }) {
  const router = useRouter()
  const [title, setTitle] = useState(pack.title)
  const [description, setDescription] = useState(pack.description)
  const [price, setPrice] = useState(String(pack.price))
  const [selected, setSelected] = useState<Set<string>>(new Set(pack.wallpaperIds))
    ...
    try {
      const res = await fetch(`/api/admin/packs/${pack.id}`, {
        ...
        body: JSON.stringify({
          title,
          description,
          price: parseFloat(price) || pack.price,
          wallpaperIds: [...selected],
        }),
      })
      const data = await res.json() as { pack?: { id: string }; error?: string }
      if (!res.ok || !data.pack) { toast.error(data.error ?? 'Failed to save'); return }
  ...
  return (
    <div className="mt-6 space-y-6">
      ...
      <div className="rounded-[4px] border border-[var(--border)] bg-[var(--surface)] p-5 space-y-4">
        ...
        {wallpapers.length === 0 ? (
          <p className="text-sm text-[var(--text-muted)]">No published wallpapers found.</p>
        ) : (
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className="rounded-[4px] border border-[var(--border)] px-5 py-2 text-sm text-[var(--text-muted)] hover:text-[var(--text)] transition-colors"
        >
          Cancel
        </button>
      </div>
