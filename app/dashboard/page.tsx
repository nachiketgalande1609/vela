...
export default async function DashboardPage() {
  ...
  const stats = [
    { icon: ShieldCheck,  label: 'Role',           value: user?.role ?? '—',          color: 'neutral' },
    { icon: MailCheck,    label: 'Email verified',  value: user?.emailVerified ? 'Verified' : 'Pending', color: user?.emailVerified ? 'green' : 'amber' },
    { icon: CalendarDays, label: 'Member since',    value: user?.createdAt.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) ?? '—', color: 'neutral' },
  ]

  const colorMap: Record<string, { bg: string; icon: string }> = {
    neutral: { bg: 'bg-neutral-100', icon: 'text-neutral-700' },
  ...
  return (
    <AppShell>
      <div className="border-b border-neutral-200 bg-white px-8 py-6">
        ...
        <p className="mt-0.5 text-sm text-neutral-500">Welcome back, {user?.name ?? user?.email}</p>
      </div>
      ...
      <div className="space-y-6 p-8">
          ...
          {stats.map(({ icon: Icon, label, value, color }) => {
            ...
            return (
              <div key={label} className="rounded-xl border border-neutral-200 bg-white p-5 shadow-sm">
                <div className={`mb-4 flex h-9 w-9 items-center justify-center rounded-xl ${c.bg}`}>
        ...
        <div className="rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
          <div className="mb-5 flex items-start gap-3">
            ...
            <div>
              <h2 className="font-semibold text-neutral-900">Session management</h2>
